...
export class RelayPool {
  ...
  setAuthSigner(signer: AuthSigner | null) {
    ...
    // Only update relays that already have a signer (i.e. explicitly authed relays)
  ...
  /** Clear the dedup set (used on sign-out / account switch). */
  ...
  addRelay(url: string): Relay {
    ...
    // NOTE: no auth signer by default — use addRelayWithAuth for outbox/inbox
  ...
  // Subscribe to specific relay URLs only (for outbox model)
  subscribeToUrls(
    urls: string[],
    filters: NostrFilter[],
    onEvent: (event: NostrEvent, relay: Relay) => void,
  ...
  private trackSeen(id: string) {
    ...
    // Prune oldest 40% when limit exceeded (Set iterates in insertion order)
